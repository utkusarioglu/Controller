...
class M_ControllerEvents {
    ...
    initialize_Controller(sequential_startup = true) {
        ...
        if (sequential_startup) {
            this.get_Controller()
                .wait_Some([
                {
                    Namespace: c_controller_1.C_Controller.AllServices,
                    Listen: c_controller_1.C_StartupTalk.run_Listen,
                    Call: () => {
    ...
    register_Services() {
        if (this._services) {
            this._services.forEach((service) => {
                this.get_Controller().respond(service.Call, service.Static || false, service.Scope || t_controller_1.e_Scope.Global, service.Group === undefined
                    ? t_controller_1.e_ServiceGroup.Standard
                    : service.Group);
            });
        }
    }
    ...
    announce_ToAllServices(resolution_instruction, delay = 0) {
        this.get_Controller()
            .announce(resolution_instruction, c_controller_1.C_Controller.AllServices, t_controller_1.e_Scope.Global, delay);
    }
    announce_LibraryAdded(library_source_namespace) {
        this.get_Controller().announce([...c_controller_1.C_BootState.LibraryAdded, [library_source_namespace]], c_controller_1.C_Controller.AllServices, t_controller_1.e_Scope.Global, true);
    }
}
